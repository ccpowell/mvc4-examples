...
Inherits="System.Web.Mvc.ViewPage<DRCOG.Domain.ViewModels.Survey.LocationViewModel>" %>

<asp:Content ID="Content1" ContentPlaceHolderID="TitleContent" runat="server">DRCOG :: Project Location</asp:Content>
...
<div class="tab-content-container">
    ...
    <div class="tab-form-container"> 
        <% using (Html.BeginForm("UpdateLocation", "Survey", FormMethod.Post, new { @id = "dataForm" })) %>
        <%{ %>
            ...
            <p><textarea class="longInputElement" name="Limits" id="Limits"><%=Model.Location.Limits%></textarea></p>
           ...
           <%foreach (DRCOG.Domain.Models.CountyShareModel cty in Model.CountyShares)
             { %>                                
                ...
                <td><%= Html.SimpleCheckBox("cty_" + cty.CountyId.ToString() + "_IsPrimary", cty.Primary.Value, "cshare_isPrimary_" + cty.CountyId.ToString(), Model.Project.IsEditable())%></td>
                <td><%= Html.DrcogTextBox("cty_" + cty.CountyId.ToString() + "_share", Model.Project.IsEditable(), (int)cty.Share.Value, new { style = "width:30px;", @id = "cshare_" + cty.CountyId.ToString() })%>%</td>
           ...
           <%if (Model.Project.IsEditable())
              { %>
                <tr id="county-editor">
                <td><%= Html.CheckBox("IsPrimary", Model.Project.IsEditable(), false, new { @id = "new_primary" })%></td>
                <td><%= Html.DrcogTextBox("Share", Model.Project.IsEditable(), 0, new { style = "width:30px;", @id = "cshare_new" })%>%</td>
           ...
           <%foreach (DRCOG.Domain.Models.MunicipalityShareModel muni in Model.MuniShares)
             { %>                                
                ...
                <td><%= Html.SimpleCheckBox("muni_" + muni.MunicipalityId.ToString() + "_IsPrimary", muni.Primary.Value, "mshare_isPrimary_" + muni.MunicipalityId.ToString(), Model.Project.IsEditable())%></td>
                <td><%= Html.DrcogTextBox("muni_" + muni.MunicipalityId.ToString() + "_share", Model.Project.IsEditable(), (int)muni.Share.Value, new { style = "width:30px;", @id = "mshare_" + muni.MunicipalityId.ToString() })%>%</td>
           ...
           <%if (Model.Project.IsEditable())
              { %>
                <tr id="muni-editor">
                <td><%= Html.CheckBox("IsPrimary", Model.Project.IsEditable(), false, new { @id = "new_primary" })%></td>
                <td><%= Html.DrcogTextBox("Share", Model.Project.IsEditable(), 0, new { style = "width:30px;", @id = "mshare_new" })%>%</td>
