...
<asp:Content ID="Content1" ContentPlaceHolderID="TitleContent" runat="server">Survey <%=Model.Current.Name%> Dashboard</asp:Content>
...
<%Html.RenderPartial("~/Views/Survey/Partials/TabPartial.ascx", Model.Current); %>
    ...
    <div class="tab-form-container">
       <h2>Summary of Projects in <% if( false /* Model.RtpStatus.IsCurrent */ ) { %>Adopted <% } %> Survey <%=Model.Current.Name%></h2>
        <label for="projectFilter">View Projects by:</label>
            <%=Html.DropDownList("projectFilter", Model.GetProjectListTypes())%>
        <br />
        <label for="projectFilterShowEmpty">Show empty items:</label>
