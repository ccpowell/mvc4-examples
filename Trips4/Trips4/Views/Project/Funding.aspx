...
<asp:Content ID="Content3" ContentPlaceHolderID="HeaderContent" runat="server">
    ...
    <script src="<%= Page.ResolveClientUrl("~/scripts/jquery.formatCurrency-1.4.0.min.js")%>" type="text/javascript" ></script>
...
<!-- FINANCIAL RECORD DETAILS -->    
        ...
        <table>
            ...
            <% foreach (System.Data.DataRow item in Model.FundingDetailPivotModel.FundingDetailTable.Rows) { 
                ...
                <% for (int j = 9; j < item.Table.Columns.Count; j++) { %>
                    <td><%= Html.DrcogTextBox("fundingdetail_" + item[1].ToString() + "_" + item[3].ToString() + "_i" + (j - 8), Model.ProjectSummary.IsInAmendment, item[j].ToString(), new { style = "width:75px;", @maxlength = "75", @class = "money", @alt = "money" })%></td>            
                <% }
                } %>
    ...
    <div>
            ...
            <a id="DeleteFundingGroup" class="delete-financialrecorddetail fg-button medium ui-state-default ui-priority-primary ui-corner-all" href="#">Delete Funding Group</a>
    ...
    <% foreach (var item in Model.FundingPhases) { %>
        ...
        <tr id="phase_row_<%= item.ProjectFinancialRecordId.ToString() %>_<%= item.FundingIncrementId.ToString() %>_<%= item.FundingResourceId.ToString() %>_<%= item.PhaseId.ToString() %>">
            ...
            <td><%= item.Year %></td>
            <td><%= item.Phase %></td>
            <td><%= item.FundingResource %></td>
            <td style="color: red; font-weight: bold;"><%= item.IsDelayed ? "YES" : "" %></td>
            <%if(Model.ProjectSummary.IsEditable()){ %>
                <td>
                    <button 
                        class="delete-phase fg-button ui-state-default ui-priority-primary ui-corner-all" 
    ...
    <%if(Model.ProjectSummary.IsEditable()){ %>
        <tr id="phase-editor">
            ...
            <td><%= Html.DropDownList("new_phase",
                    Model.ProjectSummary.IsEditable(),
                    new SelectList(Model.FundingPhasesAvailable, "key", "value", ""), 
                    "-- Phase --",
                    new { @class = "mediumInputElement", title = "Select a Phase.", @id="new_phase" })%></td>
            <td><%= Html.DropDownList("new_fundingresource",
                true,
                new SelectList(Model.ProjectFundingResources, "key", "value"),
                "-- Funding Resource --", 
                new { @class = "mediumInputElement", title="Please select a Funding Resource." })%></td>
            <td></td>
            <td><button id="add-phase" disabled='disabled' class="fg-button ui-state-default ui-priority-primary ui-state-disabled ui-corner-all">Add</button></td>
        </tr>
    <% } %>
    </table>
...
<!-- FINANCIAL DETAIL SUMMARY -->
    <div id="financial-record-detail-summary"> 
        ...
        <tr>
            ...
            <th>Future</th>
            <th>Total</th>
            <%if(Model.ProjectSummary.IsInAmendment){ %><th></th><% } %>
        </tr>
        <% foreach (var item in Model.TipProjectFunding) { %>
        <tr id="financialrecord_row_<%=item.ProjectFinancialRecordID.ToString() %>">
            <td><div id="previousTotalLabel"><%= Html.DrcogTextBox("previousTotalTextBox", Model.ProjectSummary.IsInAmendment, item.Previous.ToString(), new { style = "width:75px;", @maxlength = "75", @alt = "money" })%></div></td>
            <td><div id="programmedTotalLabel" class="currency">0</div></td>
            <td><div id="outyearTotalLabel" class="currency">0</div></td>
            <td><div id="futureTotalLabel"><%= Html.DrcogTextBox("futureTotalTextBox", Model.ProjectSummary.IsInAmendment, item.Future.ToString(), new { style = "width:75px;", @maxlength = "75", @alt = "money" })%></div></td>
            <td><div id="projectCostTotalLabel" class="currency">0</div></td>
            <%if (Model.ProjectSummary.IsInAmendment)
              { %>
        ...
        <tr>
            <td><div id="federalTotalLabel" class="currency">0</div></td>
            <td><div id="stateTotalLabel" class="currency">0</div></td>
            <td><div id="localTotalLabel" class="currency">0</div></td>
        </tr>
        </table>
        <div style="clear: both;"></div>
        <br />
        ...
        <tr id="financialrecordhistory_row_<%=item.ProjectFinancialRecordID.ToString() %>">
            <td>
                <% if (item.ProjectVersionId.ToString() == Model.ProjectSummary.ProjectVersionId.ToString()) { %>
                    Current
                <% } else  { %>
                <a href="<%= Url.Action("Details", "Project", new { tipYear = Model.ProjectSummary.TipYear, id = item.ProjectVersionId.ToString() }) %>">Details</a>
                <% } %>
            </td>
            <td><%= item.AmendmentDate.Value.Date.ToShortDateString() %></td>
            <td><%= item.AmendmentStatus.ToString() %></td>
            <td><%= Html.DrcogTextBox("financialrecordhistory_" + item.ProjectFinancialRecordID.ToString() + "_Previous", Model.ProjectSummary.IsInAmendment, item.Previous.ToString(), new { @style = "width:75px;", @maxlength = "75", @alt = "money" })%></td>
            <td><%= Html.DrcogTextBox("financialrecordhistory_" + item.ProjectFinancialRecordID.ToString() + "_Future", Model.ProjectSummary.IsInAmendment, item.Future.ToString(), new { style = "width:75px;", @maxlength = "75", @alt = "money" })%></td>
            ...
            <td><%= Html.DrcogTextBox("financialrecordhistory_" + item.ProjectFinancialRecordID.ToString() + "_FederalTotal", Model.ProjectSummary.IsInAmendment, item.FederalTotal.ToString(), new { style = "width:75px;", @maxlength = "75", @alt = "money" })%></td>
            <td><%= Html.DrcogTextBox("financialrecordhistory_" + item.ProjectFinancialRecordID.ToString() + "_StateTotal", Model.ProjectSummary.IsInAmendment, item.StateTotal.ToString(), new { style = "width:75px;", @maxlength = "75", @alt = "money" })%></td>
            <td><%= Html.DrcogTextBox("financialrecordhistory_" + item.ProjectFinancialRecordID.ToString() + "_LocalTotal", Model.ProjectSummary.IsInAmendment, item.LocalTotal.ToString(), new { style = "width:75px;", @maxlength = "75", @alt = "money" })%></td>
            <td><%= Html.DrcogTextBox("financialrecordhistory_" + item.ProjectFinancialRecordID.ToString() + "_TotalCost", Model.ProjectSummary.IsInAmendment, item.TotalCost.ToString(), new { style = "width:75px;", @maxlength = "75", @alt = "money" })%></td>
        </tr>
    <% } %>
    </table>
    </div>
