...
<div class="tab-content-container">
    ...
    <div id="details">
        <fieldset>
        <legend><%=Model.ProjectSummary.ProjectName%><%-- - <a href="#" class="printtest" title="Print Test">PRINT</a>--%></legend>
        <% if (ViewData["message"] != null) { %>
            <div id="message" class="info"><%= ViewData["message"].ToString() %></div>
        <% } %>
        <ul id="status-summary">
            <li>
                <span class="darkGrey boldFont">Begin Construction:</span>
                <%= !Model.GeneralInfo["BeginConstructionYear"].Equals(String.Empty) ? Model.GeneralInfo["BeginConstructionYear"] : "none" %>
            </li>
            <li>
                <span class="darkGrey boldFont">Open to Public:</span>
                <span id="openToPublic"><%= !Model.GeneralInfo["EndConstructionYear"].Equals(String.Empty) ? Model.GeneralInfo["EndConstructionYear"] : "none" %></span>
            </li>
                        
            <li><span class="darkGrey boldFont">Project Status:</span> <span id="projectStatus"><%= Model.GeneralInfo["ProjectStatus"]%></span></li>
            <li><span class="darkGrey boldFont">Amendment Status:</span> <span id="amendmentStatus"></span><%= Model.GeneralInfo["AmendmentStatus"] %></span></li>
        </ul>
        <div class="printarea">
        <div>
            <span class="floatLeft">Title: <span class="boldFont"><%=Model.ProjectSummary.ProjectName%></span></span>
            <span class="floatRight">Project Type: <span class="boldFont"><%= Model.GeneralInfo["ProjectType"]%></span></span>
        </div>
        <div class="clear"></div>
        <div id="summary-snapshot">
            <div>TIP-ID: <span class="boldFont"><%= Model.InfoModel.TipId %></span></div>
            <div>STIP-ID: <span class="boldFont"><%= Model.InfoModel.STIPID %></span></div>
            <div>Open to Public: <span class="boldFont"><%= !Model.GeneralInfo["EndConstructionYear"].Equals(String.Empty) ? Model.GeneralInfo["EndConstructionYear"] : "none" %></span></div>
            <div>Sponsor: <span class="boldFont"><%= Model.ProjectSponsorsModel.GetCurrent1Agency()[0].OrganizationName %></span></div>
        </div>
        <div id="scope" class="box">
            <div id="scope-info">
                <h3>Project Scope</h3>
                <p><%= Model.GeneralInfo["Scope"] %></p>
            </div>
            <div id="location-image">
                <img src='<%= Url.Action("ShowPhoto", "Project", new { id = Model.InfoModel.LocationMapId, @maxWidth = "300" }) %>' id="image_<%= Model.InfoModel.LocationMapId %>" class="resize" alt="Image map for <%= Model.InfoModel.ProjectName %>" />
            </div>
            <div class="clear"></div>
            <div id="scope-affected">
                <div id="scope-muni">
                    ...
                    <%foreach (DRCOG.Domain.Models.MunicipalityShareModel muni in Model.MuniShares) { %>                                
                        <li><%=muni.MunicipalityName%> <%= Convert.ToInt32(muni.Share.Value).ToString() %>%</li>
                    <% } %>
                    </ul>
                </div>
                <div id="scope-county">
                    ...
                    <%foreach (DRCOG.Domain.Models.CountyShareModel cty in Model.CountyShares) { %>                                
                        <li><%=cty.CountyName %> <%= Convert.ToInt32(cty.Share.Value).ToString() %>%</li>
                    <% } %>
                    </ul>
                </div>
                <div class="clear"></div>
        ...
        <table id="funding-summary">
            <tr>
                <th style="width:150px;">Amounts in $1,000s</th>
                <th style="width:80px;">Prior Funding</th>
                ...
                <% } %>
                <th style="width:80px;">Future Funding</th>
                <th style="width:80px;">Total Funding</th>
            </tr>
            <% foreach (System.Data.DataRow item in Model.FundingDetailPivotModel.FundingDetailTable.Rows) { 
                ...
                <tr id="fundingdetail_row_<%= item[1].ToString() + '_' + item[3].ToString() %>">
                    <td id="fundingdetail_<%= item[1].ToString() + '_' + item[3].ToString() %>_FundingType"><%= item[4].ToString() %> (<%= item[7].ToString() %>)</td>
            ...
            <tr>
                <td>Total</td>
                <td>$<%= Model.TipProjectFunding.Previous %></td>
                <%  int c = 3;
                    foreach (var increment in Model.FundingDetailPivotModel.FundingIncrements) {
                %>
                    <td><div id="funding-summary-<%= c.ToString() %>"></div></td>
                <% c++;
                    } %>
                <td>$<%= Model.TipProjectFunding.Future %></td>
       ...
       <% if (Model.PoolProjects.Count == 0) { %>
            There are not any pool projects for <%= Model.ProjectSummary.ProjectName %>
       <% } else { %>
...
<div style='display: none'>
    <div id='test_report'>
        ...
        <iframe id='ifrmPrintTest' src='#' style="width:0pt; height:0pt; border: none;"></iframe>
