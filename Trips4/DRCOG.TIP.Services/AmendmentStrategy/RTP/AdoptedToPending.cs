...
{
    public class AdoptedToPending : IAmendmentStrategy
    {
        protected readonly IRtpProjectRepository RtpProjectRepository;
        protected readonly CycleAmendment Amendment;

        public AdoptedToPending(IRtpProjectRepository repo, CycleAmendment amendment)
        {
        ...
        protected void Copy()
        {
            ...
            Amendment.ProjectVersionId = Amendment.Id.Equals(default(int)) 
                ? (int)RtpProjectRepository.CopyProject(Amendment.PreviousVersionId).ProjectVersionId 
                : (int)RtpProjectRepository.CopyProject(Amendment.Id, Amendment.PreviousVersionId).ProjectVersionId;
        }
